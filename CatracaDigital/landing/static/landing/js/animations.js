...
jQuery(document).ready(function($) {
    ...
    if (isMobile.any === false) { 
		...
		$('#features .ipad-holder').css('opacity', 0).one('inview', function(event, isInView) {
            if (isInView) {$(this).addClass('animated fadeInUp delayp4');}
        });
		...
		$('#how .content-left').css('opacity', 0).one('inview', function(event, isInView) {
            if (isInView) {$(this).addClass('animated fadeInLeft delayp2');}
        });
		
		$('#how .icon').css('opacity', 0).one('inview', function(event, isInView) {
